...
public class ServerHandler extends SimpleChannelInboundHandler<String> {
    ...
    protected void channelRead0(ChannelHandlerContext context, String s) throws Exception {
        ...
        for (Channel ch : group) {
            if (ch == channel) {
                ch.writeAndFlush(Protocol.buildMsg("[you]: " + s));
            } else {
                ch.writeAndFlush(Protocol.buildMsg("[" + channel.remoteAddress() + "]: " + s));
            }
        }
        System.out.println("[" + channel.remoteAddress() + "]: " + s + "\n");

        // 可以直接向群组发送消息
        //group.writeAndFlush("收到群发消息");
    }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable e) throws Exception {
        Channel channel = ctx.channel();
        System.out.println(Protocol.buildMsg("[" + channel.remoteAddress() + "] leave the room"));
        e.printStackTrace();
        ctx.close().sync();
    }
